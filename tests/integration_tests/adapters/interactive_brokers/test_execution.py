...
async def test_submit_order(
    ...
    async def mock_connect():
        ...
        exec_client._client.subscribe_event(f"accountSummary-{account}", exec_client._on_account_summary)
...
async def test_on_exec_details_uses_stored_avg_px(
    ...
    assert exec_client._order_avg_prices[cache.order(client_order_id).client_order_id] == Price.from_str("99.75")
